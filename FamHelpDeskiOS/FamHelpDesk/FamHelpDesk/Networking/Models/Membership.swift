...
struct MembershipRequest: Codable, Identifiable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // Handle the timestamp conversion
        let timestamp = try container.decode(TimeInterval.self, forKey: .requestDate)
        let date = Date(timeIntervalSince1970: timestamp)
        let formatter = ISO8601DateFormatter()
        requestDate = formatter.string(from: date)
    }
}
...
enum RequestStatus: String, Codable {
    case pending = "PENDING"
    case awaiting = "AWAITING" // API uses AWAITING, we map it to pending
