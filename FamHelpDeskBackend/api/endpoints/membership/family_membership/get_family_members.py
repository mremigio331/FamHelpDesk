...
@router.get(
    "/{family_id}/members",
    summary="Get all members for a family",
    response_description="List of all active members with user details",
)
@exceptions_decorator
def get_family_members(request: Request, family_id: str):
    """
    Get All Members for Family

    Returns all active members for the specified family.
    Each member includes the user's display name and email.

    Args:
        family_id: The family ID to get members for

    Returns:
        A JSON response containing a list of members with user details
    """
    logger.append_keys(request_id=request.state.request_id)
    logger.info(f"Getting all members for family {family_id}.")
    ...
    logger.info(f"Found {len(enriched_members)} members in family {family_id}.")
