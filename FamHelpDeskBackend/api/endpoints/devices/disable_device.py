...
@router.put(
    "/{device_id}/disable",
    summary="Disable device for push notifications",
    response_description="Updated device information",
)
@exceptions_decorator
def disable_device(request: Request, device_id: str):
    """
    Disable Device Endpoint

    Disables push notifications for a registered device without unregistering it.
    ...
    if device.user_id != user_id:
        logger.warning(
            f"User {user_id} attempted to disable device belonging to {device.user_id}"
        )
        return JSONResponse(
            content={
                "success": False,
    ...
    # Disable the device
    device_helper.disable_device(user_id, device_id)
