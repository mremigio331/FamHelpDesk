...
def run_cleanup(resources: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not family_id:
        raise ValueError("Input JSON missing 'family_id'")
    ...
    aws_logger.info(
        "Starting cleanup",
        extra={"stage": stage, "family_id": family_id},
    )

    deleted_items = delete_family_partition(table_name, family_id, aws_logger)

    return {
        "deleted_family_items": deleted_items,
    }
